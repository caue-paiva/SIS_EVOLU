...
#define maxx 1000
#define TamPop 12
//#define LRTRAINSIZE 12 //How many of the best individuals from past iterations are saved to train the linear regression model
...
void printXarray(float **arr){
 
 for (int i = 0; i < LRTRAINSIZE ; i++)
 {
    //printf(" current LR X ARRAY : %f \n", arr[i][1]);
 }
 
}

void printYarr(float *arr){

    for (int i = 0; i < LRTRAINSIZE; i++)
    {
        // printf(" current LR Y ARRAY : %f \n", arr[i]);
    }
...
void calculate_weights (){ 
  ...
  copy_Y_array();
   ...
   for (int i = 0; i < LRTRAINSIZE; i++)
 {
    printf("X array %f \n  Y array %f", LR_X_Array[i][1], LR_Y_array[i]);
 }

  float* return_arr = normalized_Lin_Regre(LR_X_Array, LR_Y_array); //Last index of the return_arr is the RMSE of the LR model
...
void ag(){
 ...
 printXarray(LR_X_Array);
 printYarr(LR_Y_array);
